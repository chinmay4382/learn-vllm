...
import os
import time
from config import KV_CACHE_BYTES, MARKER_DIR, MODEL_NAME, ensure_marker_dir
...
def main():
    ...
    hf_tps = None
    hf_time = None
    baseline_file = f"{MARKER_DIR}/hf_baseline.txt"
    ...
    ensure_marker_dir()
    with open(f"{MARKER_DIR}/vllm_baseline.txt", "w") as f:
    ...
    # Create marker
    with open(f"{MARKER_DIR}/task2_complete.txt", "w") as f:
